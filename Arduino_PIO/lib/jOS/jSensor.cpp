...
bool jSensorI2C::I2C_writeByte(uint8_t address, uint8_t subAddress, uint8_t data)
{
...
bool jSensorI2C::I2C_writeBytes(uint8_t address, uint8_t subAddress, uint8_t* data, uint8_t count)
{
...
uint8_t jSensorI2C::I2C_readByte(uint8_t address, uint8_t subAddress)
{
...
uint8_t jSensorI2C::I2C_readBytes(uint8_t address, uint8_t subAddress, uint8_t* dest, uint8_t count)
{
	byte retVal;
...
bool jSensorI2C::Bus_writeByte(uint8_t subAddress, uint8_t data) {
	return I2C_writeByte(DEVICE_ADDR, subAddress, data);
}
bool jSensorI2C::Bus_writeBytes(uint8_t subAddress, uint8_t* data, uint8_t count) {
	return I2C_writeBytes(DEVICE_ADDR, subAddress, data, count);
}
uint8_t jSensorI2C::Bus_readByte(uint8_t subAddress) {
	return I2C_readByte(DEVICE_ADDR, subAddress);
}
uint8_t jSensorI2C::Bus_readBytes(uint8_t subAddress, uint8_t* dest, uint8_t count) {
...
void jSensorI2C::printReg(String name, uint8_t reg)
{
...
uint8_t jSensorI2C::getBusError(void)
...
byte jSensorSPI::Bus_writeByte(uint8_t data) {
	
	_ThisSPI->beginTransaction();

	byte val = _ThisSPI->transfer(data);
	
	_ThisSPI->endTransaction();    

	return val;

}

void jSensorSPI::Bus_writeBytes(uint8_t* data, uint8_t count) {
	
	_ThisSPI->beginTransaction();

	//TODO replace with
	//_ThisSPI->transfer(data, count);

	for(int i = 0; i < count; i++)
		data[i] = _ThisSPI->transfer(data[i]);
	
	_ThisSPI->endTransaction();
	
}

void jSensorSPI::Bus_WriteReadByte(uint8_t* dataTX, uint16_t len_TX, uint8_t* dataRX, uint16_t len_RX)
{
	
	_ThisSPI->beginTransaction();

	for(int i = 0; i < len_TX; i++)
		_ThisSPI->transfer(dataTX[i]);
	
	for(int i = 0; i < len_RX; i++)
		dataRX[i] = _ThisSPI->transfer(0);
	
	_ThisSPI->endTransaction();
	
}

bool jSensorSPI::Bus_writeByte(uint8_t subAddress, uint8_t data) {
	
	_ThisSPI->beginTransaction();


	//Write Operation: (0)(Addr)
	_ThisSPI->transfer(((subAddress & 0x7F) | 0x00));
	_ThisSPI->transfer(data);
...
bool jSensorSPI::Bus_writeBytes(uint8_t subAddress, uint8_t* data, uint8_t count) {
...
uint8_t jSensorSPI::Bus_readByte(uint8_t subAddress) {
		
	_ThisSPI->beginTransaction();


	//Read Operation: (1)(Addr)
	_ThisSPI->transfer(((subAddress & 0x7F) | 0x80));
	uint8_t val = _ThisSPI->transfer(0x00);
...
uint8_t jSensorSPI::Bus_readBytes(uint8_t subAddress, uint8_t* dest, uint8_t count) {
...
void jSensorSPI::printReg(String name, uint8_t reg)
{
	byte val = read_reg(reg);
